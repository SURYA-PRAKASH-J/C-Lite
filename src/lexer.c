...
static TokenType keyword_mapping(const char *ident) {
  ...
  if (strcmp(ident, "false") == 0)
    return TOKEN_FALSE;
  if (strcmp(ident, "echo") == 0)
    return TOKEN_ECHO;
  if (strcmp(ident, "endl") == 0)
    return TOKEN_ENDL;
...
Token lexer_next_token(Lexer *lexer) {
  ...
  if (isalpha((unsigned char)lexer->current_char) ||
      lexer->current_char == '_') {
    int len = 0;
    while (isalnum((unsigned char)lexer->current_char) ||
           lexer->current_char == '_') {
