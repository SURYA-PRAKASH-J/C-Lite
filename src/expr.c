...
void expr_error(Parser *parser, const char *msg) {
  printf("Expression Error [line: %d, column: %d]: %s\n",
         parser->current_token.position.line,
         parser->current_token.position.column, msg);
  exit(1);
}
...
ASTNode *parse_factor(Parser *parser) {
  ...
  char buffer[256];
  sprintf(buffer, "Unexpected token in expression: %s",
          token_type_to_string(parser->current_token.type));
  expr_error(parser, buffer);
}
...
ASTNode *parse_term(Parser *parser) {
  ...
  while (parser->current_token.type == TOKEN_MULTPLY ||
...
ASTNode *parse_expression(Parser *parser) { return parse_or(parser); }
